...
function Form() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in data) {
      if (key === "file") continue; // файл отдельно обрабатывается
    ...
    <>
      <form onSubmit={handleSubmit} className="form" noValidate>
        <div className="form__file-container">
          ...
          <div className={`form__file-wrapper ${data.file ? 'form__file-wrapper--active' : ''}`}>
            ...
            <div className="form__file-button"
              onClick={() => {
                setData(oldData => ({ ...oldData, file: null }));
        ...
        <div className="form__input-group">
          ...
          /> */}
          <div className="form__input-wrapper">
            <PatternFormat className={`form__text-input ${errors.tel ? 'incorrect' : data.tel ? 'correct' : ''}`} type="tel" name="tel" value={data.tel} onChange={onChange} format="+7 (###) ###-##-##" allowEmptyFormatting mask="_" required />
            <span className="checkmark">✓</span>
          </div>
